/*!
* \file QvtkAbstractITKMeshToMeshFilter.hxx
...
#ifdef __Qvtk_ABSTRACT_ITK_MESH_TO_MESH_FILTER_H__
...
QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::~QvtkAbstractITKImageToImageFilter()
{
...
template <typename TInputMeshType, typename TOutputMeshType>
bool QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::ErrorCheck()
{
...
template <typename TInputMeshType, typename TOutputMeshType>
typename vtkPolyData* QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::GetVTKOutputData()
{
	if (!ErrorCheck())
	{
		...
		ConvertMeshTovtkPolyData(outMesh, outPD);

		QvtkAbstractFilter::Update();
...
template <typename TInputMeshType, typename TOutputMeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::UpdateOutputPort()
{
	if (!this->ErrorCheck())
	{
		qCritical() << "Failed to update output port!";
		return;
	}

	this->GetITKFilter()->Update();

	TOutputMeshType::Pointer outMesh = this->GetITKFilter()->GetOutput();

	vtkSmartPointer<vtkPolyData> outPD = vtkSmartPointer<vtkPolyData>::New();

	ConvertMeshTovtkPolyData(outMesh, outPD);

	this->GetFilter()->SetInputData(outPD);
}


template <typename TInputMeshType, typename TOutputMeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::Update()
{
	if (!ErrorCheck())
	{
        this->UpdateInputPort(); /* If there are more port that nee */
...
template <typename TInputMeshType, typename TOutputMeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::PreConnectFilter(QvtkAbstractFilter* preFilter, int prePort /*= 0*/, int thisPort /*= 0*/)
{
	try {
		this->ErrorCheck();
	}
	catch (std::runtime_error &e) /* Allows lack of input */
	{
		...
		if (NULL != castedPostFilter)
		{
			...
			itk::MeshSource<TInputMeshType>* imSource
...
template <typename TInputMeshType, typename TOutputMeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::UpdateInputPort(int thisPort)
{
    if (!this->prefilter)
    {
...
template <typename TInputMeshType, typename TOutputMeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::SetInputConnection(vtkAlgorithmOutput* input, int port /*= 0*/)
{
	if (!this->GetITKFilter())
	{
...
template <typename TInputMeshType, typename TOutputMeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::SetInputData(TInputMeshType::Pointer input)
{
	if (this->GetITKFilter())
	{
...
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::
ConvertMeshTovtkPolyData(MeshType mesh, vtkPolyData* outpd)
{
...
template<typename MeshType>
void QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::
ConvertVtkPolyDataToMesh(vtkPolyData* inpd, MeshType mesh)
{
...
template <typename TInputMeshType, typename TOutputMeshType>
typename itk::ImageToImageFilter<TInputMeshType, TOutputMeshType>* 
	QvtkAbstractITKMeshToMeshFilter<TInputMeshType, TOutputMeshType>::GetITKFilter()
{
	return (itk::ImageToImageFilter<TInputMeshType, TOutputMeshType>*)(this->itkfilter);
}
