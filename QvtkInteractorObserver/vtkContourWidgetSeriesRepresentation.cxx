...
vtkStandardNewMacro(vtkContourWidgetSeriesRepresentation);
vtkCxxSetObjectMacro(vtkContourWidgetSeriesRepresentation, ContourRepresentation, vtkContourRepresentation);
...
void vtkContourWidgetSeriesRepresentation::PrintSelf(ostream & os, vtkIndent indent)
{
}

int vtkContourWidgetSeriesRepresentation::GetNumberOfContours()
{
	return static_cast<int>(this->Contours->size());
}

vtkContourRepresentation * vtkContourWidgetSeriesRepresentation::GetContourRepresentation(unsigned int num)
{
...
int vtkContourWidgetSeriesRepresentation::GetActiveContour()
{
	return this->ActiveContour;
}

int vtkContourWidgetSeriesRepresentation::CreateContour(/*double e[2]*/)
{
	//double pos[3];
	//pos[0] = e[0];
	//pos[1] = e[1];
...
void vtkContourWidgetSeriesRepresentation::RemoveLastContour()
{
...
void vtkContourWidgetSeriesRepresentation::RemoveActiveContour()
{
	if (this->Contours->size() < 1)
	{
		return;
...
void vtkContourWidgetSeriesRepresentation::RemoveContour(int n)
{
	// Remove nth handle
...
void vtkContourWidgetSeriesRepresentation::BuildRepresentation()
{
	if (this->ActiveContour >= 0 && this->ActiveContour < static_cast<int>(this->Contours->size()))
	{
...
int vtkContourWidgetSeriesRepresentation::ComputeInteractionState(int X, int Y, int vtkNotUsed(modify))
{
	...
	for (i = 0, iter = this->Contours->begin(); iter != this->Contours->end(); ++iter, ++i)
	{
		if (*iter != NULL && (*iter)->GetNumberOfNodes() > 1)
		{
			if ((*iter)->ActivateNode(X, Y)) {
				...
				this->InteractionState = vtkContourWidgetSeriesRepresentation::NEAR_NODE;
				return this->InteractionState;
			}
			else if (PointOnContour((*iter), X, Y))
			{
				this->ActiveContour = i;
				this->InteractionState = vtkContourWidgetSeriesRepresentation::NEAR_CONTOUR;
				return this->InteractionState;
			}
		}
	}
	...
	this->InteractionState = vtkContourWidgetSeriesRepresentation::OUTSIDE;
	return this->InteractionState;
}

bool vtkContourWidgetSeriesRepresentation::PointOnContour(vtkContourRepresentation * contourRep, int X, int Y)
{
...
vtkContourWidgetSeriesRepresentation::vtkContourWidgetSeriesRepresentation()
{
